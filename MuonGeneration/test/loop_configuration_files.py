...
create_geometries = False # security flag to prevent accidental creation of geometries when not needed
...
i = 0
total_iterations = len(spacings) * len(ratios) * len(FontsSizeX)**2 * len(materials) * len(words_geometry) * len(strokes) 
                ...
                for word in words_geometry:
                    for stroke in strokes:
                        ...
                        command = [
                            ...
                            "--output_dir", PATH_geometry_files + "/" + namefile
                        ]
