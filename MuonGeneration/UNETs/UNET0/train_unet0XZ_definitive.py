...
── Usage ───────────────────────────────────────────────────────────────────────
  Edit the CONFIG section, then:
      python3 train_unet0XZ_definitive.py
...
── Output folder (one per experiment, auto-named) ──────────────────────────────
  data/models_XZ/<run_id>/
  ├── run_config.json          ← all hyperparameters, saved at start
  ├── training_log_1_XZ.csv   ← loss / PSNR / SSIM per epoch
  ├── model_summary_1_XZ.txt  ← architecture
  ├── data_indices/
  │   └── train.csv  val.csv  test.csv  ← reproducible split (x_path, y_path)
  └── checkpoints/
      ├── best_model_1_XZ.keras
      ├── last_checkpoint_1_XZ.keras
      └── epoch_monitoring/
          └── UNET1XZ_epochNNN_monitoring.png
...
MODELS   = DATA / "models_XZ"

# ── Projection (fixed for this script — do not change) ──────────────────────
_PROJECTION  = "XZ"   # projection name, used in labels and run_config.json
_CH_IDX      = 1      # channel index in the (128, 128, 3) NPY arrays
_MODEL_LABEL = "1_XZ" # suffix used in checkpoint file names
...
class NPYDataGenerator:
    """Streams XZ channel from NPY files with optional per-epoch subsampling."""
...
def _visualize_split(model, pairs, split, n_samples, output_dir):
    ...
    plt.suptitle(f"UNET1XZ — {split} samples", fontsize=12, y=1.01)
    plt.tight_layout()
...
def main():
    ...
    print("=" * 80)
    print(f" UNET2D-XZ TRAINING — POCA Denoising, XZ channel ({LOSS_FN} loss)")
    ...
    if EVAL_ONLY:
        ...
        if VISUALIZE_RESULTS:
            viz_dir = OUTPUT_DIR / "visualizations_XZ"
    ...
    print(f"[MODEL] Architecture     : UNET2D standard (single-channel XZ)")
...
class EpochMonitoringCallback(keras.callbacks.Callback):
    ...
    def _load_sample(self, pair):
        """Load a (x_path, y_path) pair, extract XZ channel, return (x, y)."""
        x_path, y_path = pair
        x_data = np.load(x_path, allow_pickle=False).astype(np.float32)
        y_data = np.load(y_path, allow_pickle=False).astype(np.float32)
        if FLUX_FRACTION > 0:
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        fig.suptitle(
            f"UNET1XZ  ·  Epoch {epoch_num:03d} / {self.params['epochs']}"
        ...
        out_path = self.monitoring_dir / f"UNET1XZ_epoch{epoch_num:03d}_monitoring.png"
