...
Input: .npy files from folder(s) — extracts channel 0 (XY projection)
Output: best model saved to models_XY/checkpoints/best_model_0_XY.keras
...
Configure: RUN_SELECTION, SUBSAMPLE_FRACTION at top of script.
"""
...
def _x_to_y_path(x_path, gt_folder):
    ...
    X: tensor_2D_POCA_<stem>[_Muons_100000_2D].npy
    Y: tensor_2D__<stem>.npy  (double underscore — JSON stem starts with '_')
    """
...
def main():
    ...
    print(f"[MODEL] Loss / metrics   : Charbonnier / PSNR, SSIM(5×5)")
